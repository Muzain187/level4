...
bool hasCycle_DFS(vector<vector<char>> &board,char col,int pi,int pj,int i,int j,int n,int m,bool **visited){
    ...
    if(i+1<n && board[i+1][j]==col && !visited[i+1][j])
        ans=ans || hasCycle_DFS(board,col,i,j,i+1,j,n,m,visited);
    
    if(j+1<m && board[i][j+1]==col && !visited[i][j+1])
        ans=ans || hasCycle_DFS(board,col,i,j,i,j+1,n,m,visited);
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++){
            ...
            if(!visited[i][j]){
                ...
                ans=hasCycle_DFS(board,color,i,j,i,j,n,m,visited);
                initialize(visited,n,m);
                if(ans)
                    return 1;
            }
